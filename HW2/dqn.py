...
class DQN:
    ...
    def update(self, buffer):
        ...
        Q = self.Q.forward(s)
        Q_tar = self.Q_tar.forward(sp)

        if not done[0]:
            y_i = r + self.discount*torch.max(Q_tar, dim=1)[0]
        else:
            y_i = r

        Q_val = torch.gather(Q, 0, a)

        loss = self.Q.loss(y_i, Q_val)
        loss.backward()
        self.optimizer_Q.step()

        if((self.C % self.training_step) == 0):
            Q_tar = Q
